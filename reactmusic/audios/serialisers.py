...
from rest_framework.serializers import (
    ModelSerializer, HyperlinkedModelSerializer, ImageField, BooleanField, CharField, ValidationError, IntegerField
)

from mutagen.mp3 import MP3, HeaderNotFoundError

from reactmusic.utils.custom_exceptions import SongDoesNotExist
from reactmusic.artists.models import Artist
from reactmusic.artists.parsers.artist_parser import ArtistParser
...
class AudioCreateSerializer(ModelSerializer):
    ...
    class Meta:
        model = Audio
        fields = ['id', 'name', 'owner', 'playcount', 'has_lyrics', 'album', 'audio_file', 'mbid', 'artist', 'tags', 'extra_sm_image_thumbnail', 'lyrics', 'image', 'bitrate', 'length']
    ...
    def validate(self, attrs):
        ...
        stdlogger.warning("starting validation: {}".format(raw_file.name))
        # codecs hell
        try:
            try:
        ...
        stdlogger.warning("finishing validation: {}".format(raw_file.name))
        return attrs
