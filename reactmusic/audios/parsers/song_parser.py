...
class SongParser:
    ...
    def _parse_info(self):
        ...
        try:
            ...
            url = "https://ws.audioscrobbler.com/2.0/?method=track.getInfo&api_key={key}&artist={artist}&track={name}&autocorrect=1&format=json".format(key=settings.LAST_FM_KEY, artist=self.artist, name=self.song)
            data = self.s.get(url)
            try:
            ...
            try:
                ...
                pass
            try:
                image_url = next(filter(lambda cover: cover['size'] == 'extralarge', data_dict['album']['image']))['#text']
            ...
            url = "https://ws.audioscrobbler.com/2.0/?method=artist.getinfo&artist={artist}&api_key={key}&autocorrect=1&format=json".format(key=settings.LAST_FM_KEY, artist=self.artist)
            data = self.s.get(url)
            try:
                ...
                try:
                    ...
                    try:
                        image_url = next(filter(lambda cover: cover['size'] == 'extralarge', data_dict['image']))['#text']
