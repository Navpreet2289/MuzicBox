...
class TagListView(ListAPIView):
    ...
    def get_queryset(self):
        if self.request.user.is_authenticated:
            ...
            qs = Tag.objects.filter(audio__owner=self.request.user).active().annotate(audios_count=Count(
                'audio')).order_by('-audios_count')[:30]    # top 30 tags
        else:
            # returning tags from presentation songs
            qs = Tag.objects.for_presentation().annotate(audios_count=Count(
                'audio')).order_by('-audios_count')[:30]    # top 30 tags
        return qs
...
class AudioListCreateView(ListCreateAPIView):
    ...
    def order_queryset(self, qs):
        ...
        if sort_filter == 'popularity':
            ...
            qs = cache_users_audios(
                qs, self.request.user.id, tag_name=self.get_tag_filter())
        else:
    ...
    def get_queryset(self):
        if self.request.user.is_authenticated:
            ...
            qs = Audio.objects.filter(owner=self.request.user).select_related(
                "artist").prefetch_related("tags")
        else:
            # returning presentational audios
            qs = Audio.objects.for_presentation().select_related(
                "artist").prefetch_related("tags")
    ...
    def simple_search(self, song, qs):
        qs = qs.filter(Q(name__icontains=song) | Q(
            artist__name__icontains=song)).distinct()
        if qs.exists():
            return qs
    ...
    def complex_search(self, song, qs):
        ...
        search_artist = reduce(
            operator.or_, (Q(artist__name__icontains=x) for x in song_parts))
        search_song = reduce(operator.or_, (Q(name__icontains=x)
                                            for x in song_parts))
