...
from reactmusic.audios.models import Audio
...
def cache_users_audios(qs, user_id, tag_name=None):
    ...
    if not cached_dict['ids'] or tag_name != cached_dict['tag']:
        ...
        cached_ids_list = list(qs.order_by('?').values_list(
            'id', flat=True))   # not efficent :)
        cached_dict = {'ids': cached_ids_list, 'tag': tag_name}
        cache.set("user_{}_audios_random".format(user_id),
                  cached_dict, settings.AUDIO_CACHING_TIMEOUT)

    preserved = Case(*[When(pk=pk, then=pos)
                       for pos, pk in enumerate(cached_dict['ids'])])
    cached_audios = Audio.objects.filter(
        pk__in=cached_dict['ids']).order_by(preserved).select_related(
