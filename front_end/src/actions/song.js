...
import { setPlaylistBackup, setPlaylist } from "./playlist";
    ...
    SongApi.getNextSongs()
      .then(songs_object => {
        ...
        if (songs_object.results.length > 0) {
          ...
          dispatch(setPlaylistBackup(songs_object)); // loading songs in to backup playlist
          dispatch(setPlaylist(songs_object)); // loading songs in to playlist
          dispatch(playNext(songs_object.results[0].id)); // setting active first song during onLoad
        } else {
