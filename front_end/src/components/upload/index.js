...
class Upload extends Component {
  ...
  setProgress(e) {
    let combined_percent =
      (e.percent + 100 * this.files_sent) / this.state.accepted.length;
      ...
      if (!err) {
        ...
        if (!this.props.search_song_value) {
          if (
            ...
            data.body.tags.includes(this.props.filter_tag_value.id)
          ) {
        ...
        this._addNotification(
          `Uploaded: ${data.body["artist"]["name"]} - ${data.body["name"]}`,
          "success"
        );
      ...
      <div className="app__container">
        ...
        <div>
          <p
            ...
            style={{
              visibility:
                this.state.accepted.length && !this.state.sending
                  ? "visible"
                  : "hidden"
            }}
          >
            you&#39;ve selected {this.state.accepted.length}{" "}
            {this.state.accepted.length === 1 ? "file" : "files"}
          </p>
          {!this.state.sending ? (
            <div styleName="button__cotainer">
              <button
                disabled={this.state.disabled}
                className={sendButtonCls}
  ...
  return bindActionCreators(
    {
      setSongs: setSongs,
      noSongs: noSongs,
      initialLoadSongs: initialLoadSongs,
